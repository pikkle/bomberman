...
public class InsertHandler extends Observable {
    ...
    private void registerDragEvent() {
        src.getScene().getRoot().setOnDragOver(new EventHandler<DragEvent>() {
    ...
    private void registerDropEvent() {
        dst.setOnDragOver(new EventHandler<DragEvent>() {
            @Override
            public void handle(DragEvent t) {
                try {
                    if(!(clazz.newInstance() instanceof PowerUp) || renderer.getArena().getElements(renderer.getCell(t.getX(), t.getY())).stream().filter(element -> element instanceof Box).findFirst().isPresent()){
        ...
        dst.setOnDragExited(new EventHandler<DragEvent>() {
            @Override
            public void handle(DragEvent t) {
                dst.setEffect(null);
                t.consume();
        ...
        dst.setOnDragDropped(new EventHandler<DragEvent>() {
            @Override
            public void handle(DragEvent t) {
                ...
                if (db.getString().equals("insert")) {
                    try {
                        ...
                        Element element = (Element) constructor.newInstance(renderer.getCell(t.getX(), t.getY()), renderer.getArena());
                        setChanged();
    ...
    private <T extends Element> Constructor<T> findConstructor(Class<T> dataClass){
